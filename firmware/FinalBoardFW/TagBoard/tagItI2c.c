...
void setPWM(uint8_t dutycycle, uint16_t freq){
  ...
  sl_pwm_init(&pwm_buzzer, &pwm_config);
  sl_pwm_set_duty_cycle(&pwm_buzzer, dutycycle);
  sl_pwm_start(&pwm_buzzer);
}

void stopPWM(void){
  sl_pwm_deinit(&pwm_buzzer);
}
